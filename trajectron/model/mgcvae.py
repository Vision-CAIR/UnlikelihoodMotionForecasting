...
from model.safty_checker import safety_checker, visualization_unsafe, visualization_lane_direction
import model.dynamics as dynamic_module
from utils.trajectory_utils import traj_fit_patch
...
class MultimodalGenerativeCVAE(object):
    ...
    def set_annealing_params(self):
        ...
        self.create_new_scheduler(name='gamma',
                                  annealer=sigmoid_anneal,
                                  annealer_kws={
                                      'start': 0,
                                      'finish': self.hyperparams.get('gamma', 1),
                                      'center_step': self.hyperparams.get('gamma_crossover', -10),
                                      'steps_lo_to_hi': 500
                                  })
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples):
        ...
        y_dist, _ = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
    ...
    def decoder_safe(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples, num_candidate, inputs, safe_map):
        """
        Decoder of the CVAE.
        ...
        num_components = self.hyperparams['N'] * self.hyperparams['K']
        y_dist, a_dist = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
                                     prediction_horizon, num_samples, num_components=num_components)
        ...
        # negative data
        if safe_map is None:
            unlikelihood = torch.zeros_like(log_p_y_xz)
        else:
            a_sample = a_dist.rsample(torch.Size([num_candidate]))
            traj_candidate = self.dynamic.integrate_samples(a_sample, x).detach().transpose(2, 3)
            traj_candidate.requires_grad = False
            orig_candidate_shape = traj_candidate.shape

            traj_candidate = traj_candidate.view(-1, *orig_candidate_shape[2:])
            candidate_patch = self.traj2patch(inputs, traj_candidate)
            labels_patch = self.traj2patch(inputs, labels)

            negative_mask_orig, negative_step_mask = safety_checker(candidate_patch, safe_map)
            gt_mask = ~safety_checker(labels_patch, safe_map)[0]
            negative_mask = negative_mask_orig * gt_mask  # filter out the data where gt is dangerous (fault data)

            # for visualization and debug
            # visualization_unsafe(candidate_patch, safe_map, negative_mask, max=30)
            # visualization_unsafe(candidate_patch, safe_map, ~negative_mask, max=30)

            negative_traj_total = traj_candidate.transpose(1,0)[negative_mask.transpose(1,0)]   # need to make the batch dim first to fetch the data
            ...
            zero = torch.zeros(max_num, *traj_candidate.shape[-2:], device=self.device)

            negative_batch = [traj.repeat(n, 1, 1)[:max_num] if n <= max_num else zero for n, traj in zip(repeat_time, negative_traj)]
            negative_batch = torch.stack(negative_batch, dim=1)

            log_p_yt_xz_neg = torch.clamp(y_dist.log_prob(negative_batch),
                                          max=self.hyperparams['log_p_yt_xz_max'])

            log_p_yt_xz_neg = log_p_yt_xz_neg.mean(dim=0, keepdim=True)

            if self.hyperparams['log_histograms'] and self.log_writer is not None:
                self.log_writer.add_histogram('%s/%s' % (str(self.node_type), 'neg_log_p_yt_xz'), log_p_yt_xz_neg, self.curr_iter)

            log_p_y_xz_neg = log_p_yt_xz_neg.sum(dim=2)

            if self.hyperparams['unlike_type'] == 'lin':
                unlikelihood = log_p_y_xz_neg.exp()
            elif self.hyperparams['unlike_type'] == 'log':
                unlikelihood = (log_p_y_xz_neg.exp() + self.hyperparams['log_bias']).log()
            elif self.hyperparams['unlike_type'] == 'no':
                unlikelihood = torch.zeros_like(log_p_y_xz_neg)
            else:
                raise KeyError

            unlikelihood = unlikelihood * (num_negative > 0).float()

        return log_p_y_xz, unlikelihood

    def traj2patch(self, inputs, traj):
        ...
        rot_mat = \
            torch.stack([torch.stack([torch.cos(initial_node_head), torch.sin(initial_node_head)]),
                         torch.stack([-torch.sin(initial_node_head), torch.cos(initial_node_head)])]).permute(2, 0, 1)
        patch_size = self.hyperparams['map_encoder'][self.node_type.name]['patch_size']

        traj_patch = traj_fit_patch(traj, initial_node_pos, rot_mat, patch_size)
        return traj_patch
    ...
    def train_loss(self,
        ...
        log_p_y_xz, p_y_xz_neg = self.decoder_safe(mode, x, x_nr_t, y, y_r, n_s_t0, z,
                                                       labels,  # Loss is calculated on unstandardized label
                                                       prediction_horizon,
                                                       self.hyperparams['k'], 20, inputs, safe_map)
        likelihood_neg = p_y_xz_neg.mean()
        ...
        if self.log_writer is not None:
            ...
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'log_likelihood'),
                                       log_likelihood,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'neg_log_likelihood'),
                                       likelihood_neg,
                                       self.curr_iter)
    ...
    def eval_loss(self,
        ...
        y_dist, _, _ = self.p_y_xz(ModeKeys.PREDICT, x, x_nr_t, y_r, n_s_t0, z,
    ...
    def predict(self,
        ...
        _, _, our_sampled_future = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
                                               prediction_horizon,
                                               num_samples,
                                               num_components,
                                               gmm_mode)
