...
def homography_warp(patch_src: torch.Tensor,
    ...
    warper = HomographyWarper(x_min, y_min, x_max, y_max, out_height, out_width, mode, padding_mode, device)
...
def transform_warp_impl(src: torch.Tensor, centers: torch.Tensor, dst_pix_trans_src_pix: torch.Tensor,
                        ...
                        grid_mode: str, padding_mode: str, device) -> torch.Tensor:
    ...
    return homography_warp(src, centers, src_norm_trans_dst_norm, dsize_dst, grid_mode, padding_mode, device)
...
class HomographyWarper(nn.Module):
    ...
    def __init__(
        ...
        self.grid: torch.Tensor = create_batched_meshgrid(x_min, y_min, x_max, y_max, height, width, device=self.device)
...
def warp_affine_crop(src: torch.Tensor, centers: torch.Tensor, M: torch.Tensor,
                ...
                padding_mode: str = 'zeros', device='cpu') -> torch.Tensor:
    ...
    # launches the warper
    h, w = src.shape[-2:]
    return transform_warp_impl(src, centers, M_3x3, (h, w), dsize, flags, padding_mode, device)