...
class GeometricMap(Map):
    ...
    def get_padded_map(self, padding_x, padding_y, device):
        if self._last_padding == (padding_x, padding_y):
            ...
            self._last_padded_map = torch.full((self.data.shape[0],
                                               ...
                                               False, dtype=torch.uint8, device=device)
            self._last_padded_map[..., padding_x:-padding_x, padding_y:-padding_y] = self.torch_map(device)
            return self._last_padded_map

    @staticmethod
    def batch_rotate(map_batched, centers, angles, out_height, out_width, device='cpu'):
        """
        ...
        rotated_map_batched = warp_affine_crop(map_batched, centers, M,
                                               dsize=(out_height, out_width), padding_mode='zeros', device=device)
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        if rotation is not None:
            angles = rotation #torch.tensor(rotation, device=device)
        else:
...
class SafeMap(GeometricMap):
    ...
    def get_cropped_maps_from_scene_map_batch(cls, maps, scene_pts, patch_size, rotation=None, device='cpu'):
        ...
        centers = torch.tensor([s_map.to_map_points(scene_pts[np.newaxis, i]) for i, s_map in enumerate(maps)],
                               dtype=torch.long, device=device).squeeze(dim=1) \
                  + torch.tensor([context_padding_x, context_padding_y], device=device,
                                 dtype=torch.long)  # the location of scene point in the paded map
        ...
        center_patches = torch.tensor([[context_padding_y, context_padding_x]],
                                      # the center of patch. patch size here is 2 * context_padding
                                      dtype=torch.int,
                                      device=device).repeat(batch_size, 1)
        ...
        rotated_map_batched = cls.batch_rotate(padded_map_batched,
                                               center_patches.float(),
                                               angles,
                                               long_size,
                                               lat_size,
    ...
    def get_padded_map(self, padding_x, padding_y, device):
        if self._last_padding == (padding_x, padding_y):
            ...
            self._last_padded_map = torch.full((self.data.shape[0],
                                                self.data.shape[1] + 2 * padding_x,
                                                self.data.shape[2] + 2 * padding_y),
                                               False, dtype=torch.get_default_dtype(), device=device)
            self._last_padded_map[..., padding_x:-padding_x, padding_y:-padding_y] = self.torch_map(device)
            return self._last_padded_map
    ...
    def torch_map(self, device):
        if self._torch_map is not None:
            return self._torch_map
        self._torch_map = torch.tensor(self.data, device=device)
        return self._torch_map
    ...
    def batch_rotate(map_batched, centers, angles, out_height, out_width, device='cpu'):
        ...
        rot_mat = torch.stack([torch.stack([torch.cos(angles_rad), torch.sin(angles_rad)], dim=1),
                               torch.stack([-torch.sin(angles_rad),  torch.cos(angles_rad)], dim=1)], dim=1)

        rot_mat = rot_mat.view(len(angles_rad), 1, 1, 2, 2)
        direction = rotated_map_batched[:, :2].permute(0, 2, 3, 1).unsqueeze(-1)
        rotated_direction = torch.matmul(rot_mat, direction).squeeze(-1).permute(0, 3, 1, 2)
        rotated_map_batched[:, :2] = rotated_direction
